...
static void start_dwell_timer (MTClosure *mt);
static void stop_timer	      (GTimer *timer, guint *tid);

static void
show_debug (gchar *msg)
{
    gchar *str;
    static int c = 1;
...
debug_window (void)
{
    ...
    gtk_window_set_title (GTK_WINDOW(win), "MouseTweaks");
    view = gtk_text_view_new ();
    buffer = gtk_text_view_get_buffer (GTK_TEXT_VIEW(view));
    gtk_text_buffer_set_text (buffer, "Debugging Console\n\n", -1);
    gtk_container_add (GTK_CONTAINER(win), view);
    gtk_window_set_keep_above (GTK_WINDOW(win), TRUE);
    gtk_widget_set_size_request (win, 280, 120);
    //gtk_widget_show_all (win);
}

static void
mt_set_grab_cursor (gboolean set)
{
    GdkCursor *cursor;
    GdkWindow *root;
...
static gboolean
analyze_direction (MTClosure *mt)
{
    GestureDirection gd;
    gint i, x, y, dx, dy;
...
dwell_start_gesture (MTClosure *mt)
{
    ...
    start_dwell_timer (mt);
}

static void
dwell_stop_gesture (MTClosure *mt)
{
    stop_timer (mt->dwell_timer, &mt->dwell_tid);
    mt->dwell_gesture_started = FALSE;

    gdk_pointer_ungrab (GDK_CURRENT_TIME);

    if (analyze_direction (mt)) {
	/* gesture: context menu workaround */
	if (mt->dwell_cct == DWELL_CLICK_TYPE_RIGHT)
	    mt->context_menu_opened = TRUE;

	dwell_do_pointer_click (mt, mt->pointer_x, mt->pointer_y);

	if (mt->context_menu_opened) {
...
/* timeout functions */
static gboolean
dwell_check_time (gpointer data)
{
    MTClosure *mt = (MTClosure *) data;

    if (g_timer_elapsed (mt->dwell_timer, NULL) > mt->dwell_time) {
	gint x, y;
...
static gboolean
delay_check_time (gpointer data)
{
    MTClosure *mt = (MTClosure *) data;

    if (g_timer_elapsed (mt->delay_timer, NULL) > mt->delay_time) {
...
/* timer stuff */
static void
start_dwell_timer (MTClosure *mt)
{
    g_timer_start (mt->dwell_timer);

    if (!mt->dwell_tid)
	mt->dwell_tid = g_timeout_add (100,
				       (GSourceFunc) dwell_check_time,
				       (gpointer) mt);
}

static void
start_delay_timer (MTClosure *mt)
{
    g_timer_start (mt->delay_timer);

    if (!mt->delay_tid)
	mt->delay_tid = g_timeout_add (100,
				       (GSourceFunc) delay_check_time,
				       (gpointer) mt);
}

static void
stop_timer (GTimer *timer, guint *tid)
{
    g_timer_stop (timer);
    g_timer_reset (timer);
...
spi_motion_event (const AccessibleEvent *event, void *data)
{
    ...
    if (mt->delay_tid)
	if (!within_tolerance (mt, event->detail1, event->detail2))
	    stop_timer (mt->delay_timer, &mt->delay_tid);
}
...
spi_button_event (const AccessibleDeviceEvent *event, void *data)
{
    ...
    if (mt->delay_enabled && event->keycode == 1) {
	switch (event->type) {
		 ...
		 gpointer data)
{
	...
	mt->delay_time = gconf_value_get_float (value);
	...
	mt->dwell_time = gconf_value_get_float (value);
    else if (g_str_equal (key, OPT_CTW)) {
	mt->dwell_show_ctw = gconf_value_get_bool (value);
	mt_ctw_update_visible (mt->dwell_show_ctw);
    }
...
mt_gconf_read_options (MTClosure *mt)
{
    ...
    mt->delay_time = gconf_client_get_float (mt->client, OPT_DELAY_T, NULL);
    mt->dwell_enabled = gconf_client_get_bool (mt->client, OPT_DWELL, NULL);
    mt->dwell_time = gconf_client_get_float (mt->client, OPT_DWELL_T, NULL);
...
mt_closure_init (void)
{
    ...
    mt = g_new0 (MTClosure, 1);
    if (!mt)
	return NULL;

    mt->delay_timer = g_timer_new ();
    mt->dwell_timer = g_timer_new ();
...
{
    g_timer_destroy (mt->dwell_timer);
    g_timer_destroy (mt->delay_timer);
...
const char *
daemon_pid_file_proc_mt (void)
{
    static char fn[PATH_MAX];

    snprintf (fn, sizeof(fn), "%s/.%s.pid",
	      g_get_home_dir (), daemon_pid_file_ident);
...
main (int argc, char **argv)
{
    ...
    daemon_pid_file_ident = "mousetweaks";
    daemon_pid_file_proc = daemon_pid_file_proc_mt;
