...
void
mt_ctw_update_visible (gboolean visible)
{
    if (visible)
	gtk_widget_show_all (glade_xml_get_widget (xml, "ctw"));
    else
...
mt_ctw_update_sensitivity (MTClosure *mt)
{
    ...
    if (mt->dwell_enabled && mt->dwell_mode == DWELL_MODE_CTW)
	sensitive = TRUE;
...
mt_ctw_init (MTClosure *mt, gint x, gint y)
{
    ...
    xml = glade_xml_new (GLADE_PATH "/ctw.glade", NULL, NULL);
    if (!xml) {
