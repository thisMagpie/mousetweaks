...
#define DWELL_ENABLED_KEY	  \
    MT_GCONF_HOME "/dwell_enabled"

#define DWELL_MODE_KEY	       \
		    ...
		    const char *cname)
{
    ...
    if (dd->active)
	send_dbus_signal (dd->conn, SHOW_DIALOG_SIGNAL, 1);
}
...
{
    DwellMode mode;
    gboolean enabled, sensitive = FALSE;

    enabled = gconf_client_get_bool (dd->client, DWELL_ENABLED_KEY, NULL);
    mode = gconf_client_get_int (dd->client, DWELL_MODE_KEY, NULL);

    if (dd->active && enabled && mode == DWELL_MODE_CTW)
	sensitive = TRUE;
...
gconf_value_changed (GConfClient *client,
		     ...
		     gpointer data)
{
    if (g_str_equal (key, DWELL_MODE_KEY) ||
	g_str_equal (key, DWELL_ENABLED_KEY))
...
fill_applet (PanelApplet *applet)
{
    ...
    dd->xml = glade_xml_new (GLADE_PATH "/dwell-click-applet.glade",
			     NULL, NULL);
    if (!dd->xml) {
	fini_dwell_data (dd);
	return FALSE;
	...
	gdk_pixbuf_new_from_file (GLADE_PATH "/human-single.png", NULL);
    dd->click[DWELL_CLICK_TYPE_DOUBLE] =
	gdk_pixbuf_new_from_file (GLADE_PATH "/human-double.png", NULL);
    dd->click[DWELL_CLICK_TYPE_DRAG] =
	gdk_pixbuf_new_from_file (GLADE_PATH "/human-drag.png", NULL);
    dd->click[DWELL_CLICK_TYPE_RIGHT] =
	gdk_pixbuf_new_from_file (GLADE_PATH "/human-right.png", NULL);
